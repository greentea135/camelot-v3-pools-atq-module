...
const SUBGRAPH_URLS: Record<string, { decentralized: string }> = {
  ...
  "42161": {
    decentralized:
      "https://gateway.thegraph.com/api/[api-key]/subgraphs/id/3utanEBA9nqMjPnuQP1vMCCys6enSM3EawBpKTVwnUw2",
  },
};
...
function transformPoolsToTags(chainId: string, pools: Pool[]): ContractTag[] {
  ...
  return validPools.map((pool) => {
    ...
    return {
      ...
      "Project Name": "Camelot v3",
      "UI/Website Link": "https://camelot.exchange/",
      "Public Note": `The liquidity pool contract on Camelot v3 for the ${pool.token0.name} (${pool.token0.symbol}) / ${pool.token1.name} (${pool.token1.symbol}) pair.`,
    };
  });
}
